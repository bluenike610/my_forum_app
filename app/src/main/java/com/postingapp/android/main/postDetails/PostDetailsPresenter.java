...
import com.postingapp.android.managers.CommentManager;
...
class PostDetailsPresenter extends BasePresenter<PostDetailsView> {
    ...
    private void initLikeButtonState() {
        ...
        if (profile != null && post != null) {
            postManager.hasCurrentUserLike(context, post.getId(), profile.getId(), exist -> {
                ifViewAttached(view -> {
...
//            view.setTitle(post.getTitle());
    ...
    public boolean hasAccessToModifyPost() {
        Profile profile = cm.getMyInfo();
        return profile != null && post != null && post.getAuthorId().equals(profile.getId());
    }

    public boolean hasAccessToEditComment(String commentAuthorId) {
        Profile profile = cm.getMyInfo();
        return profile != null && commentAuthorId.equals(profile.getId());
    }
